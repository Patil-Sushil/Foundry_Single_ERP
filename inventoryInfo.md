...
- **Date Handling**: Reports use `LocalDate` for filtering, defaulting to the current month or day if not provided.
- **WAC Calculation**: The Item Ledger and Stock Summary reports rely on the Weighted Average Cost (`avgRate`) maintained in real-time during every inward transaction.
- **Data Aggregation**: Business logic for reports is centralized in `InventoryReportService`, utilizing stream-based aggregation and DTOs for structured responses.
